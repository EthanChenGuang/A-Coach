...
**Note:** If you already have the `workouts` table with `estimated_duration` as INTEGER, run the migration script to update it to NUMERIC:

```sql
-- Run this migration to change estimated_duration from INTEGER to NUMERIC(5,2)
...
Or use the migration file: `server/migrations/001_alter_estimated_duration_to_numeric.sql`
