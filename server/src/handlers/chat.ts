...
import { HandlerContext, RouteHandler } from '../types';
...
export const chatHandler: RouteHandler<any> = {
  async post(context: HandlerContext<{ message: string; history?: ChatMessage[] }>) {
    try {
      ...
      console.log('Chat handler - Received body:', JSON.stringify(body));

      if (!body) {
        console.error('Chat handler - Body is missing');
        throw new Error('Request body is required');
      }

      if (!body.message || typeof body.message !== 'string') {
        console.error('Chat handler - Invalid message:', body.message);
