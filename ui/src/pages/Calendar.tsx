...
import { ScheduleMealDialog } from '@/components/diet/ScheduleMealDialog';
import { Workout } from '@/types/workout';
import { WorkoutSchedule, MealSchedule } from '@/types/schedule';
...
export default function Calendar() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const loadWorkouts = async () => {
        try {
          const data = await serverComm.getWorkouts();
      ...
      const loadMealPlans = async () => {
        try {
          const data = await serverComm.getMealPlans();
      ...
      const loadSchedules = async () => {
        try {
          const data = await serverComm.getScheduledWorkouts();
      ...
      const loadMealSchedules = async () => {
        try {
          const data = await serverComm.getScheduledMeals();
          console.log('Fetched meal schedules:', data);
          setMealSchedules(data);
        } catch (error) {
          console.error('Failed to load meal schedules:', error);
          // Don't show toast for meal schedules as backend might not be implemented yet
        }
      };

      // Load all in parallel but handle errors independently
  ...
  const handleScheduleMeal = async (schedule: Omit<MealSchedule, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) => {
    try {
      const newSchedule = await serverComm.scheduleMeal(schedule);
  ...
  const handleDeleteMealSchedule = async (scheduleId: string) => {
    try {
      await serverComm.deleteScheduledMeal(scheduleId);
      ...
      toast({
        title: 'Error',
        description: 'Failed to delete meal schedule',
        variant: 'destructive'
      });
    }
  };
  ...
  const getMealSchedulesForDay = (date: Date) => {
    if (!Array.isArray(mealSchedules)) return [];
    return mealSchedules.filter(schedule => {
      const scheduleDate = new Date(schedule.date);
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-7 gap-px bg-muted">
          {days.map((day) => {
            ...
            const dayMealSchedules = getMealSchedulesForDay(day);
            ...
            return (
              ...
              >
                ...
                <div className="mt-2 space-y-1">
                  ...
                  {dayMealSchedules.map((schedule) => {
                    const mealPlan = mealPlans.find(mp => mp.id === schedule.mealPlanId);
                    return mealPlan ? (
                      <div
                        key={schedule.id}
                        className="group flex items-center justify-between gap-1 rounded-md bg-green-100 dark:bg-green-900/20 px-2 py-1 text-xs"
                      >
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDeleteMealSchedule(schedule.id);
                          }}
                        >
                          <Trash2 className="h-3 w-3" />
                        </Button>
